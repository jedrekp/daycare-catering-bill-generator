...
public interface ChildRepository extends JpaRepository<Child, Long> {
    ...
    @Query("SELECT c FROM Child c LEFT JOIN FETCH c.preschoolGroup WHERE c.id = :childId")
    Optional<Child> findByIdWithPreschoolGroup(@Param("childId") Long childId);

    @Query("SELECT c FROM Child c LEFT JOIN FETCH c.chosenDiets cd  left join fetch cd.diet where c.id = :childId")
